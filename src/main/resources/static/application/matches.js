...
function getmatchesbypage(page) {
    ...
    $(".loader").show();
    var jqxhr = $.get( "/matches?page="+page+"&size=10", function(data) {
            console.log(data);
            $("table#matches_table").find("tbody").html("");
            for(var i=0;i<data.content.length;i++) {
                var str = "<tr>";

                var date = new Date(data.content[i].dtMatch);
                var strDate = date.toGMTString()+"";

                str+="<td>"+data.content[i].tournamenttext+"</td>";
                str+="<td>"+strDate+"</td>";
                str+="<td><object data='/images/"+data.content[i].teamid1+".png' type='image/png' class='imgintable'><img class='imgintable' src='/images/3326728.jpg'/></object><br/>"+getTeamNameById(data.content[i].teamid1)+"</td>";
                str+="<td> VS </td>";
                str+="<td><object data='/images/"+data.content[i].teamid2+".png' type='image/png' class='imgintable'><img class='imgintable' src='/images/3326728.jpg'/></object><br/>"+getTeamNameById(data.content[i].teamid2)+"</td>";
                str+="<td><object data='/images/"+data.content[i].winner+".png' type='image/png' class='imgintable'><img class='imgintable' src='/images/3326728.jpg'/></object><br/>"+getTeamNameById(data.content[i].winner)+"</td>";
